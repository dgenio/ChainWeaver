...
jobs:
  validate:
    ...
    steps:
      ...
      - uses: dgenio/ChainWeaver/.github/actions/chainweaver@v0.14.1
        with:
          directory: flows/
```
...
| `chainweaver-version` | `0.14.1` | Exact version of `chainweaver` to install from PyPI (passed through to `pip install "chainweaver==<version>"`). Pinned by default; pass an empty string for the latest published version. |
...
(`@v0.14.1`) rather than `@main` to avoid implicit upgrades.
