...
├── claude.py          Claude Code adapter: normalize_claude_hook_event(s) PostToolUse→AgentTraceEvent (#272), render_posttooluse_hook + add/remove_observe_hook_from_settings (#271), add/remove_flow_server_from_config → mcpServers (#273), ClaudeCodeAdapterError (CW-E053); offline, banned from executor.py
├── vscode.py          VS Code / Copilot adapter: normalize_vscode_event(s) MCP trace→AgentTraceEvent (#265), copilot_otel_settings_snippet (printed, not written), add/remove_flow_server_from_config → servers (#269), VSCodeAdapterError (CW-E054); offline, banned from executor.py
├── _agent_config.py   Private editor-agnostic MCP-config helpers: add/remove_flow_server(servers_key=...) + backup_file, shared by opencode/claude/vscode adapters; banned from executor.py
...
├── cli/               typer-based CLI command package (#333): inspect/viz (ascii/dot/mermaid + --result overlay, #392), explain (deterministic LLM-free review render, #420), init (project scaffolder, #441), validate/check/dump-schema, run/serve, profile, diff, attest, suggest, record, flows (promote/ignore/list), traces (mine/draft-flows/backtest), opencode group (`opencode capture` plugin event→trace JSONL #276; `opencode setup`/`revert` reversible observe-plugin + FlowServer exposure with dry-run/backups #277/#279/#280), claude group (`claude capture` PostToolUse hook→trace JSONL #272; `claude setup`/`revert` reversible observe-hook + `.mcp.json` FlowServer exposure with scope/dry-run/backups #271/#273), vscode group (`vscode capture` MCP trace→JSONL from stdin/--from #265; `vscode setup`/`revert` prints Copilot OTel observe snippet + reversible `.vscode/mcp.json` FlowServer exposure #265/#269), doctor group (`doctor flow` --check-drift / --preflight / --profile first-run #442; `doctor vscode|claude|opencode` read-only coding-agent workspace inspectors #264/#270/#275), fuzz, service
