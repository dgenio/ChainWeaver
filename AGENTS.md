...
- `FlowExecutor(..., checkpointer=..., delete_on_success=True)` → crash-resume (#128); writes an `ExecutionSnapshot` after every successful linear step or DAG level. Snapshot-write failures are caught and logged at `WARNING` (chainweaver.executor) — the flow continues without that snapshot, mirroring the middleware-hook-exception semantics so persistence bugs are never more catastrophic than tool bugs. `FlowExecutor.resume_flow(trace_id)` validates the snapshot's flow version and tool `schema_hash` values against the current registry — drift raises `CheckpointDriftError` — then continues execution with the original `trace_id`. Snapshots are deleted on terminal success when `delete_on_success=True` (the default); preserved on failure for operator-driven retry.
