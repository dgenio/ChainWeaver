...
def _looks_like_tool_event(payload: Mapping[str, Any], tool: Any) -> bool:
    """Heuristically decide whether *payload* describes a tool execution.

    A payload counts as a tool event when it names a tool, or when its event
    ``type`` / ``event`` discriminator mentions ``tool`` (e.g.
    ``"tool.execute.after"``).  Pure model / bus events are skipped.
    """
    if isinstance(tool, str) and tool:
        return True
    discriminator = _first(payload, "type", "event")
    return isinstance(discriminator, str) and _TOOL_EVENT_HINT in discriminator.lower()
...
def normalize_opencode_event(
    ...
    *,
    ...
    Returns:
        The normalized event, or ``None`` when *payload* does not describe a
        tool execution (e.g. an unrelated bus event), so callers can filter it.
    ...
    if not _looks_like_tool_event(payload, tool):
        return None
    ...
    metadata = {key: value for key, value in payload.items() if key not in _KNOWN_OPENCODE_KEYS}
...
def render_observe_plugin(*, sink: str = OPENCODE_TRACE_SINK, redact: bool = True) -> str:
    ...
    Args:
        sink: Workspace-relative trace sink path baked into the plugin.
